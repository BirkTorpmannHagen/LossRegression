...
class EndoCV2020(data.Dataset):
    ...
    def __getitem__(self, idx):
        mask = Image.open(self.mask_locs[idx])
        image = Image.open(self.img_locs[idx])
...
def build_polyp_datasets_partitioned(root, img_size=512):
    trans = alb.Compose([
            alb.Resize(img_size, img_size)])
    ind_train = KvasirSegmentationDataset(join(root, "HyperKvasir"), trans, trans, split="train")
    ind_val = KvasirSegmentationDataset(join(root, "HyperKvasir"), trans, trans, split="val")
    ind_test = KvasirSegmentationDataset(join(root, "HyperKvasir"), trans, trans, split="test")
    etis = EtisDataset(join(root, "ETIS-LaribDB"), trans, split="train")
    cvc = CVC_ClinicDB(join(root, "CVC-ClinicDB"), trans, split="train")
    endocv = EndoCV2020(join(root, "EndoCV2020"), trans)
    return ind_train, ind_val, ind_test, etis, cvc, endocv  